...
public class WomenClothingController {
    ...
    @RequestMapping(value = "/selectionclothing/{brand}/{color}/{price}/{categoryItem}", method = RequestMethod.GET)
    public
    @ResponseBody
    String getAllSelectClothing(@PathVariable("brand") String brand, @PathVariable("color") String color, @PathVariable("price") String price,
        ...
        try {
            List<PWomenClothing> finalProd = dataServices.getAllWomenSelectClothingList(brand, color, maxPrice, minPrice, categoryItem);
            Gson gson = new Gson();
